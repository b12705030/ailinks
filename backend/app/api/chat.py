...
- 用清晰、結構化的方式呈現信息
- 如果有多個連結，用編號列表（1. 2. 3.）來展示
- 對於統計信息，用具體數字和分類來回答
- 例如：「你總共有 **50** 個連結，其中 **健身** 類有 5 個，**學習** 類有 10 個...」

請用友好、簡潔的中文回答。如果找到相關連結，請在回答中提及並總結。如果用戶問統計問題，請使用上面的統計信息回答。"""
            }
        ]
...
2. 如果有多個連結，用編號列表展示（1. 2. 3.）
3. 提供具體信息：標題、分類、標籤、來源等
4. 如果用戶問統計問題，使用統計信息回答，並用 **粗體** 標出數字和分類
5. 讓回答更具體、更有用

例如格式：
「我找到了 **3** 個關於 **健身** 的連結：
1. 【健身】標題 - 來自 Instagram，標籤：#運動 #健康
2. 【健身】標題 - 來自 Messenger，標籤：#訓練
3. 【健身】標題 - 來自 Facebook，標籤：#營養」"""
